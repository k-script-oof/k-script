...
local NATIVE_DB_HASH = "e2f8a9295019aaf3dc5d299e9dcdcb55c7d6d4bfdcc2abca6a2b833ebec461ec"
local AUTH_SECRET = "bwGWpJcBgLfWZ1URLQBVjmX0druSlYuxAmJSBn05iWIlGVlyGz6f4R0QNG1hjw"
local AUTH_WAIT_MS = 15000
local AUTH_YIELD_MS = 10
local TERMS_ACCEPTED_FILE_NAME = "\\K-Script_Terms_Accepted.txt"
...
K_SCRIPT_EVENT_IDS = {
  ...
  DONATOR = 785347088,
  HOST_SHARE = 351121409,
  MODERATOR = 813963773,
  UNLOAD = 295410189,
  USER = 422501215,
}

local K_SCRIPT_EVENT_BASE = 1073741824
local K_SCRIPT_EVENT_MOD = 1073740823
local K_SCRIPT_EVENT_SALT = 146026062

local function encodeKScriptEventValue(rockstarId, eventId)
  rockstarId = math.floor(tonumber(rockstarId) or 0)
  eventId = math.floor(tonumber(eventId) or 0)

  local ridPart = rockstarId * 97
    + (rockstarId % 104729) * 8191
    + (rockstarId % 8191) * 65537
  local eventPart = eventId * 31337 + (eventId % 65521) * 127

  return math.floor(K_SCRIPT_EVENT_BASE + ((ridPart + eventPart + K_SCRIPT_EVENT_SALT) % K_SCRIPT_EVENT_MOD))
end
...
K_SCRIPT_RULES_TEXT = [[K-Script Rules & Regulations
...
No Protecting "Lexis" Users: Abusing the K-Script user detection system to shield or protect Lexis users is forbidden.

Agreement: By using K-Script and its supporter features, you automatically agree to follow these rules. Admin enforcement is final.
]]

local TERMS_UNLOCK_SECONDS = 20.0
local TERMS_FADE_SECONDS = 0.42

local TERMS_RULE_SECTIONS = {
  {
  ...
  {
    title = "03  User Detection Conduct",
    body = {
      "Do not target or harass K-Script/Cherax users based on the detection system.",
      "Do not abuse detection data to shield or protect Lexis users.",
    },
  },
  {
    title = "04  Agreement",
    body = {
      "Using K-Script means you agree to these rules. Admin enforcement is final.",
    },
  },
}

termsAccepted = FileMgr.DoesFileExist(TERMS_ACCEPTED_FILE)
termsError = nil
termsDeclined = false
termsDialogActive = false
termsDialogStartedAt = nil
termsDecisionFadeActive = false
termsDecisionFadeStart = nil
termsDecisionFadeAction = nil
...
function EaseSmootherStep(value)
  value = Clamp(value, 0.0, 1.0)
...
function EaseOutBack(value)
  ...
  local c1 = 1.70158
  local c3 = c1 + 1.0
  local shifted = value - 1.0

  return 1.0 + (c3 * shifted * shifted * shifted) + (c1 * shifted * shifted)
end

function Alpha255(alpha, opacity)
  return math.floor(Clamp(alpha * opacity, 0, 255))
end
...
function CaptureMouseAndDisableControls()
...
local function popStyleColors(count)
  for _ = 1, count do
    ImGui.PopStyleColor()
  end
end

local function startTermsDecisionFade(action)
  if termsDecisionFadeActive then
    return
  end

  termsDecisionFadeActive = true
  termsDecisionFadeAction = action
  termsDecisionFadeStart = ImGui.GetTime()
end

local function termsSocialClubName()
...
local function drawTermsCursor(opacity)
  local x, y = ImGui.GetMousePos()
  local shadowAlpha = Alpha255(235, opacity)
  local fillAlpha = Alpha255(250, opacity)
  local accentAlpha = Alpha255(90, opacity)

  ImGui.AddLine(x - 1, y - 1, x - 1, y + 20, 0, 0, 0, shadowAlpha, 3)
  ImGui.AddLine(x - 1, y - 1, x + 14, y + 11, 0, 0, 0, shadowAlpha, 3)
  ImGui.AddLine(x + 14, y + 11, x + 8, y + 12, 0, 0, 0, shadowAlpha, 3)
  ImGui.AddLine(x + 8, y + 12, x + 12, y + 20, 0, 0, 0, shadowAlpha, 3)
  ImGui.AddLine(x + 12, y + 20, x + 8, y + 22, 0, 0, 0, shadowAlpha, 3)
  ImGui.AddLine(x + 8, y + 22, x + 4, y + 14, 0, 0, 0, shadowAlpha, 3)
  ImGui.AddLine(x + 4, y + 14, x - 1, y + 20, 0, 0, 0, shadowAlpha, 3)

  ImGui.AddLine(x, y, x, y + 18, 236, 255, 244, fillAlpha, 1.6)
  ImGui.AddLine(x, y, x + 12, y + 10, 236, 255, 244, fillAlpha, 1.6)
  ImGui.AddLine(x + 12, y + 10, x + 6, y + 11, 236, 255, 244, fillAlpha, 1.6)
  ImGui.AddLine(x + 6, y + 11, x + 10, y + 19, 236, 255, 244, fillAlpha, 1.6)
  ImGui.AddLine(x + 10, y + 19, x + 8, y + 20, 236, 255, 244, fillAlpha, 1.6)
  ImGui.AddLine(x + 8, y + 20, x + 4, y + 12, 236, 255, 244, fillAlpha, 1.6)
  ImGui.AddLine(x + 4, y + 12, x, y + 18, 236, 255, 244, fillAlpha, 1.6)
  ImGui.AddLine(x + 1, y + 2, x + 10, y + 10, 84, 190, 120, accentAlpha, 2.6)
end

local function terminalLinesForTerms(name)
  local prompt = tostring(name or "user") .. "@K-Script:$ "
  return {
    { text = prompt .. "whoami", color = { 88, 255, 166, 245 } },
    { text = tostring(name or "user"), color = { 214, 219, 225, 230 } },
    { text = prompt .. "kscriptctl session --verify", color = { 88, 255, 166, 245 } },
    { text = "[ ok ] identity token verified", color = { 130, 235, 165, 238 } },
    { text = prompt .. "mount /rules --tos", color = { 88, 255, 166, 245 } },
    { text = "[ ok ] ruleset mounted read-only", color = { 130, 235, 165, 238 } },
    { text = "[ wait ] decision gate armed", color = { 245, 198, 104, 238 } },
  }
end

local function drawTerminalPreview(x, y, width, height, name, elapsed, opacity)
  ImGui.AddRectFilled(x, y, x + width, y + height, 4, 5, 7, Alpha255(244, opacity), 8)
  ImGui.AddRectFilled(x, y, x + width, y + 28, 23, 24, 27, Alpha255(250, opacity), 8)
  ImGui.AddRectFilled(x, y + 22, x + width, y + 28, 23, 24, 27, Alpha255(250, opacity), 0)
  ImGui.AddRectFilled(x + 11, y + 10, x + 19, y + 18, 255, 92, 92, Alpha255(240, opacity), 4)
  ImGui.AddRectFilled(x + 27, y + 10, x + 35, y + 18, 255, 199, 86, Alpha255(230, opacity), 4)
  ImGui.AddRectFilled(x + 43, y + 10, x + 51, y + 18, 96, 224, 130, Alpha255(225, opacity), 4)

  local lineY = y + 44
  local reveal = math.max(0, elapsed - 0.18)
  for _, line in ipairs(terminalLinesForTerms(name)) do
    local chars = math.min(#line.text, math.floor(reveal * 150))
    if chars > 0 then
      local color = line.color
      ImGui.AddText(x + 16, lineY, line.text:sub(1, chars), color[1], color[2], color[3], Alpha255(color[4], opacity))
      lineY = lineY + 17
    end

    if chars < #line.text then
      break
    end

    reveal = reveal - (#line.text / 150) - 0.12
  end
end

local function drawRuleSummary(x, y, width, opacity)
  local currentY = y

  for _, section in ipairs(TERMS_RULE_SECTIONS) do
    ImGui.AddRectFilled(x, currentY, x + width, currentY + 34 + (#section.body * 18), 18, 20, 24, Alpha255(238, opacity), 8)
    ImGui.AddRectFilled(x, currentY, x + 5, currentY + 34 + (#section.body * 18), 84, 190, 120, Alpha255(220, opacity), 5)
    ImGui.AddText(x + 15, currentY + 10, section.title, 244, 246, 248, Alpha255(248, opacity))

    local bodyY = currentY + 30
    for _, line in ipairs(section.body) do
      ImGui.AddText(x + 15, bodyY, line, 185, 190, 196, Alpha255(226, opacity))
      bodyY = bodyY + 18
    end
...
function HasAcceptedTerms()
  return FileMgr.DoesFileExist(TERMS_ACCEPTED_FILE)
end

function AcceptTerms()
  FileMgr.CreateDir(K_SCRIPT_DIR)

  local content = K_SCRIPT_RULES_TEXT .. "\nAccepted: true\n"
  local ok, result = pcall(function()
    return FileMgr.WriteFileContent(TERMS_ACCEPTED_FILE, content)
  end)

  if not ok or result == false then
    termsError = ("Failed to save acceptance file: %s"):format(tostring(result))
    Logger.LogError("[K-Script Terms] " .. termsError)
    return false
  end
  ...
  termsError = nil
  ReleaseMouseCapture()
  Logger.Log(eLogColor.GREEN, "K-Script", "Terms of service accepted.")
  return true
end
...
function DeclineTerms()
  termsDeclined = true
  ReleaseMouseCapture()
  Logger.LogError("[K-Script Terms] Terms of service declined, unloading...")
  SetShouldUnload()
end

function WaitForTermsAccepted()
  if HasAcceptedTerms() then
    return true
  end

  termsDialogActive = true
  termsDialogStartedAt = nil
  termsDecisionFadeActive = false
  termsDecisionFadeStart = nil
  termsDecisionFadeAction = nil

  while not HasAcceptedTerms() and not ShouldUnload() do
    Script.Yield(50)
  end

  termsDialogActive = false
  ReleaseMouseCapture()
  return HasAcceptedTerms() and not ShouldUnload()
end

function DrawTermsDialog()
  if not termsDialogActive or HasAcceptedTerms() or ShouldUnload() then
    return false
  end

  CaptureMouseAndDisableControls()

  local now = ImGui.GetTime()
  if termsDialogStartedAt == nil then
    termsDialogStartedAt = now
  end

  if termsDecisionFadeActive and termsDecisionFadeStart ~= nil and now - termsDecisionFadeStart >= TERMS_FADE_SECONDS then
    if termsDecisionFadeAction == "accept" then
      if not AcceptTerms() then
        termsDecisionFadeActive = false
        termsDecisionFadeStart = nil
        termsDecisionFadeAction = nil
      end
  ...
  local elapsed = now - termsDialogStartedAt
  local unlocked = elapsed >= TERMS_UNLOCK_SECONDS and not termsDecisionFadeActive
  local progress = math.min(elapsed / TERMS_UNLOCK_SECONDS, 1.0)
  local opacity = EaseSmootherStep(math.min(elapsed / 0.62, 1.0))

  if termsDecisionFadeActive and termsDecisionFadeStart ~= nil then
    opacity = 1.0 - EaseInCubic((now - termsDecisionFadeStart) / TERMS_FADE_SECONDS)
  end

  opacity = Clamp(opacity, 0.02, 1.0)

  local screenWidth, screenHeight = ImGui.GetDisplaySize()
  local cardWidth = math.min(math.max(screenWidth * 0.58, 840), screenWidth - 80)
  local cardHeight = math.min(math.max(screenHeight * 0.54, 550), 625)
  local x = (screenWidth - cardWidth) * 0.5
  local y = (screenHeight - cardHeight) * 0.5
  local padding = 24
  local headerHeight = 74
  local footerHeight = 72
  local accentR, accentG, accentB = 84, 190, 120
  ...
  if ImGui.Begin("##KScriptTermsAgreement", true, flags) then
    local windowX, windowY = ImGui.GetWindowPos()
    ImGui.AddRectFilled(0, 0, screenWidth, screenHeight, 0, 0, 0, Alpha255(105, opacity), 0)
    ImGui.AddRectFilled(x - 10, y - 10, x + cardWidth + 10, y + cardHeight + 10, 0, 0, 0, Alpha255(95, opacity), 16)
    ImGui.AddRectFilled(x, y, x + cardWidth, y + cardHeight, 12, 14, 18, Alpha255(248, opacity), 12)
    ImGui.AddRectFilled(x + 1, y + 1, x + cardWidth - 1, y + headerHeight, 20, 22, 28, Alpha255(248, opacity), 12)
    ImGui.AddLine(x + 22, y + headerHeight, x + cardWidth - 22, y + headerHeight, accentR, accentG, accentB, Alpha255(165, opacity), 2)

    ImGui.AddText(x + padding, y + 20, "K-Script Terms", 248, 250, 252, Alpha255(255, opacity))
    ImGui.AddText(x + padding, y + 43, "Review required before the authenticated splash continues", 164, 170, 178, Alpha255(238, opacity))
    ImGui.AddRectFilled(x + cardWidth - 170, y + 22, x + cardWidth - padding, y + 52, 30, 33, 39, Alpha255(236, opacity), 7)
    ImGui.AddText(x + cardWidth - 157, y + 30, termsSocialClubName() .. "@K-Script", 156, 255, 190, Alpha255(238, opacity))

    local contentTop = y + headerHeight + 18
    local contentBottom = y + cardHeight - footerHeight - 14
    local leftWidth = math.max(260, math.min(330, cardWidth * 0.36))
    local rightX = x + padding + leftWidth + 18
    local rightWidth = cardWidth - (rightX - x) - padding

    ImGui.AddRectFilled(x + padding, contentTop, x + padding + leftWidth, contentBottom, 17, 19, 24, Alpha255(238, opacity), 10)
    ImGui.AddText(x + padding + 16, contentTop + 18, "Session Check", 238, 241, 245, Alpha255(245, opacity))
    ImGui.AddText(x + padding + 16, contentTop + 40, "Terminal preview", 136, 143, 153, Alpha255(224, opacity))
    drawTerminalPreview(x + padding + 14, contentTop + 72, leftWidth - 28, 184, termsSocialClubName(), elapsed, opacity)

    local unlockY = contentTop + 292
    ImGui.AddText(x + padding + 16, unlockY, "Decision unlock", 220, 224, 229, Alpha255(235, opacity))
    ImGui.AddRectFilled(x + padding + 16, unlockY + 24, x + padding + leftWidth - 16, unlockY + 31, 35, 38, 44, Alpha255(245, opacity), 4)
    ImGui.AddRectFilled(x + padding + 16, unlockY + 24, x + padding + 16 + ((leftWidth - 32) * progress), unlockY + 31, accentR, accentG, accentB, Alpha255(245, opacity), 4)
    if unlocked then
      ImGui.AddText(x + padding + 16, unlockY + 45, "Ready for your decision", 156, 255, 190, Alpha255(235, opacity))
    else
      ImGui.AddText(x + padding + 16, unlockY + 45, ("Unlocks in %.1fs"):format(math.max(TERMS_UNLOCK_SECONDS - elapsed, 0)), 170, 176, 184, Alpha255(225, opacity))
    end

    ImGui.AddText(rightX, contentTop, "Rules Summary", 244, 246, 248, Alpha255(250, opacity))
    ImGui.AddText(rightX, contentTop + 22, "These are the active K-Script usage terms.", 148, 154, 164, Alpha255(228, opacity))
    drawRuleSummary(rightX, contentTop + 44, rightWidth, opacity)

    local footerY = y + cardHeight - footerHeight
    ImGui.AddRectFilled(x + 1, footerY, x + cardWidth - 1, y + cardHeight - 1, 16, 18, 23, Alpha255(248, opacity), 12)
    ImGui.AddRectFilled(x + 1, footerY, x + cardWidth - 1, footerY + 8, 16, 18, 23, Alpha255(248, opacity), 0)

    local status = "Read the summary before choosing."
    if unlocked then
      status = "Choose how to continue. Accept stores the terms file in your K-Script folder."
    elseif termsDecisionFadeActive then
      status = "Closing..."
    end
    if termsError ~= nil then
      status = termsError
    end

    ImGui.AddText(x + padding, footerY + 20, status, 196, 202, 210, Alpha255(236, opacity))

    local buttonWidth = 188
    local declineWidth = 90
    local buttonHeight = 34
    local buttonGap = 10
    local buttonX = x + cardWidth - padding - buttonWidth - declineWidth - buttonGap
    local buttonY = footerY + 18

    ImGui.PushStyleVar(ImGuiStyleVar.Alpha, opacity)
    ImGui.SetCursorPosX(buttonX - windowX)
    ImGui.SetCursorPosY(buttonY - windowY)
    ImGui.PushStyleColor(ImGuiCol.Text, 0.96, 1.0, 0.97, 1.0)
    ImGui.PushStyleColor(ImGuiCol.Button, 0.08, unlocked and 0.5 or 0.25, 0.24, 0.98)
    ImGui.PushStyleColor(ImGuiCol.ButtonHovered, 0.1, 0.68, 0.32, 1.0)
    ImGui.PushStyleColor(ImGuiCol.ButtonActive, 0.06, 0.8, 0.36, 1.0)
    if ImGui.Button("ACCEPT TERMS", buttonWidth, buttonHeight) and unlocked then
      startTermsDecisionFade("accept")
    end
    ImGui.SameLine()
    popStyleColors(4)

    ImGui.PushStyleColor(ImGuiCol.Text, 1.0, 0.96, 0.96, 1.0)
    ImGui.PushStyleColor(ImGuiCol.Button, 0.54, 0.09, 0.11, 0.98)
    ImGui.PushStyleColor(ImGuiCol.ButtonHovered, 0.76, 0.1, 0.14, 1.0)
    ImGui.PushStyleColor(ImGuiCol.ButtonActive, 0.95, 0.06, 0.1, 1.0)
    if ImGui.Button("DECLINE", declineWidth, buttonHeight) then
      startTermsDecisionFade("decline")
    end
    popStyleColors(4)
    ImGui.PopStyleVar()

    drawTermsCursor(opacity)
  end
...
function DownloadAndSaveLuaAssets(url, fileName)
  ...
  curl:Setopt(eCurlOption.CURLOPT_USERAGENT, "K-Script V25.69")
...
local function isHexString(value, length)
  value = tostring(value or "")
  return #value == length and value:match("^[0-9a-fA-F]+$") ~= nil
end
...
function DoLoaderChallengeOrExit(baseUrl)
  ...
  if type(decodedChallenge) ~= "table" then
    FuckTheniggersGame()
    return
  end

  local challenge = tostring(decodedChallenge.challenge or decodedChallenge.c or ""):lower():gsub("%s+", "")
  local challengeExpiresAt = tostring(decodedChallenge.challengeExp or decodedChallenge.e or ""):gsub("%s+", "")
  local build = tostring(decodedChallenge.build or decodedChallenge.b or "")

  if not isHexString(sid, 32) or not isHexString(challenge, 64) or challengeExpiresAt == "" then
    os.exit(69)
    FuckTheniggersGame()
    return
  end

  local proof = __K_CHAL_RESP(challengeExpiresAt, challenge, build)
  proof = tostring(proof or ""):lower():gsub("%s+", "")
  if not isHexString(proof, 64) then
    proof = tostring(__K_CHAL_RESP(challenge, challengeExpiresAt) or ""):lower():gsub("%s+", "")
  end
  if not isHexString(proof, 64) then
    os.exit(69)
    FuckTheniggersGame()
  ...
  local verifiedToken = type(decodedVerify) == "table" and (decodedVerify.scriptToken or decodedVerify.token or decodedVerify.t) or nil
  if type(decodedVerify) ~= "table" or decodedVerify.ok ~= true or type(verifiedToken) ~= "string" or verifiedToken == "" then
    os.exit(69)
    FuckTheniggersGame()
    return
  end

  scriptToken = verifiedToken
...
discordButtons = {
  ...
  { label = "Subscribe to Script", url = "https://cherax.menu/scripts/3" },
}

local DISCORD_CLIENT_ID = "1413655927617359973"
...
local function permissionBool(value)
...
local function singleUidList(enabled, uid)
  return enabled and { uid } or {}
end

local function permissionTier(admin, moderator, supporter)
  if admin then
    return 3
  end
...
function FetchUserPermissions(yieldWhileWaiting)
  ...
  local permissions = decodedOk and type(decoded) == "table" and decoded.permissions or nil
  if type(permissions) ~= "table" and decodedOk and type(decoded) == "table" then
    permissions = decoded
  end
  ...
  if type(permissions) == "table"
    and (
      permissions.isBanned ~= nil
      or permissions.isAdmin ~= nil
      or permissions.isModerator ~= nil
      or permissions.isSupporter ~= nil
    )
  then
    local permissionIsAdmin = permissionBool(permissions.isAdmin)
    local permissionIsModerator = permissionBool(permissions.isModerator)
    local permissionIsSupporter = permissionIsAdmin or permissionIsModerator or permissionBool(permissions.isSupporter)

    AdmanUIDs = singleUidList(permissionIsAdmin, uid)
    moderatorUIDs = singleUidList(permissionIsModerator, uid)
    supporterUIDs = singleUidList(permissionIsSupporter, uid)
    bannedUIDs = singleUidList(permissionBool(permissions.isBanned), uid)
  else
  ...
  if tableContainsValue(bannedUIDs, uid) then
    Logger.Log(eLogColor.RED, "K-Script", "You`ve been Banned from using K-Script!")
    if type(SendToDiscord) == "function" then
      SendToDiscord(
        "lol",
        ("Banned User tried to load K-Script, Banned UID: %i, SC Name: %s"):format(uid, SOCIALCLUB.SC_ACCOUNT_INFO_GET_NICKNAME())
      )
    end
  ...
  local previousTier = permissionTier(wasAdmin, wasModerator, wasSupporter)

  isAdmin = tableContainsValue(AdmanUIDs, uid)
  isModerator = tableContainsValue(moderatorUIDs, uid)
  isSupporter = isAdmin or isModerator or tableContainsValue(supporterUIDs, uid)

  local currentTier = permissionTier(isAdmin, isModerator, isSupporter)
  if logOnce and previousTier ~= currentTier then
    local direction = previousTier > currentTier and "downgrade" or "upgrade"
    local message = ("Permission %s detected, please reload the Script!\nUnloading..."):format(direction)
    GUI.AddToast("K-Script", message, 20000, eToastPos.TOP_RIGHT)
    Logger.Log(eLogColor.GREEN, "K-Script", (message:gsub("\n", " ")))
    SetShouldUnload()
    return
  end
...
function AuthenticateToAPI(clientName)
  ...
  if tostring(rockstarId):find("Error:", 1, true) then
    rockstarId = tostring(1234567890)
  end
  ...
  while not curl:GetFinished() do
    if os.clock() - startedAt >= 8 then
      GUI.AddToast("K-Script", "Authentication failed: Request timed out after 8 seconds.\nContact a K-Script Developer", 20000, eToastPos.TOP_RIGHT)
...
function HardwareCheck(payload)
  local function request(requestPayload)
    ...
    addAuthHeaders(curl)
    curl:AddHeader("User-Agent: K-Script V25.69")
    curl:Setopt(eCurlOption.CURLOPT_POST, 1)
  ...
  local function request(requestKind, requestContent, requestSocialClubName, requestRockstarId)
    ...
    local payload = {
      discord_payload = {
        ...
        avatar_url = "https://c.wallhere.com/photos/5b/87/2560x1600_px_Grand_Theft_Auto_V_logo-564561.jpg!d",
      },
...
local function buildDiscordActivity()
  ...
  if numConnectedPlayers > 0 then
    state = ("In a Session with %d players"):format(numConnectedPlayers)
  else
    state = "In Story Mode"
  end
...
local function featureIsToggledSafe(feature)
  if feature == nil then
    return false
  end

  if feature.IsToggled then
    local ok, enabled = pcall(feature.IsToggled, feature)
    if ok then
      return enabled == true
    end
  end

  if feature.GetHash and FeatureMgr.IsFeatureEnabled then
    local hashOk, hash = pcall(feature.GetHash, feature)
    if hashOk and hash ~= nil then
      local enabledOk, enabled = pcall(FeatureMgr.IsFeatureEnabled, hash)
      if enabledOk then
        return enabled == true
      end
    end
...
local function disableOnlineJoinLeaveNotifications()
  local notifyJoin = FeatureMgr.GetFeatureByName("Notify On Player Join")
  local notifyLeave = FeatureMgr.GetFeatureByName("Notify On Player Leave")
  ...
  local function disableFeature(feature)
    ...
    if feature.SetBoolValue then
      local ok = pcall(feature.SetBoolValue, feature, false)
      if not ok and feature.SetValue then
        pcall(feature.SetValue, feature, false)
      end
    elseif feature.SetValue then
      pcall(feature.SetValue, feature, false)
    end
  ...
  disableFeature(notifyJoin)
  disableFeature(notifyLeave)
end

local function getOnlineLookupRockstarId()
  local notifyJoin = FeatureMgr.GetFeatureByName("Notify On Player Join")
  local notifyLeave = FeatureMgr.GetFeatureByName("Notify On Player Leave")

  if (tonumber(numConnectedPlayers) or 0) <= 1 or not (featureIsToggledSafe(notifyJoin) or featureIsToggledSafe(notifyLeave)) then
    return getLocalRockstarId()
  end

  local localPlayerId = GTA.GetLocalPlayerId()
  for playerId = 0, 31 do
    if playerId ~= localPlayerId then
      local playerOk, player = pcall(Players.GetById, playerId)
      if playerOk and player ~= nil then
        local gamerInfo
        if player.GetGamerInfo then
          local infoOk, info = pcall(player.GetGamerInfo, player)
          if infoOk then
            gamerInfo = info
          end
        end

        if gamerInfo and gamerInfo.RockstarId then
          return tostring(gamerInfo.RockstarId)
        end

        disableOnlineJoinLeaveNotifications()
        return getLocalRockstarId()
      end
    end
  end
...
function GetOnlineUserInfo()
  local function request()
    ...
    curl:Setopt(eCurlOption.CURLOPT_POSTFIELDS, json.encode({
      ...
      rockstar_id = getOnlineLookupRockstarId(),
      be_enabled = tostring(isBattlEyeEnabled(Cherax.GetEdition())),
    }))
    curl:Setopt(eCurlOption.CURLOPT_POST, 1)
    curl:Setopt(eCurlOption.CURLOPT_USERAGENT, "K-Script V25.69")
    curl:DisableErrorLog()
...
SCRIPT_EVENT = {
  ...
  SCRIPT_EVENT_FM_EVENT_GIVE_WANTED_LEVEL = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_FM_EVENT_GIVE_WANTED_LEVEL"),
  USING_CHAT_WINDOW = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_OHD_USING_CHAT_WINDOW"),
  USING_CHAT_WINDOW_RESET = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_OHD_USING_CHAT_WINDOW_RESET"),
  CONFIRMATION_LAUNCH_MISSION = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_CONFIRMATION_LAUNCH_MISSION"),
  SCRIPT_EVENT_CONFIRMATION_LAUNCH_MISSION = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_CONFIRMATION_LAUNCH_MISSION"),
  SCRIPT_EVENT_INVITE_PLAYER_ONTO_MISSION = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_INVITE_PLAYER_ONTO_MISSION"),
  SCRIPT_EVENT_INVITE_TO_MISSION = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_INVITE_PLAYER_ONTO_MISSION"),
  INVITE_TO_MISSION = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_INVITE_PLAYER_ONTO_MISSION"),
  SCRIPT_EVENT_FORCE_PLAYER_ONTO_MISSION = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_FORCE_PLAYER_ONTO_MISSION"),
  FORCE_PLAYER_ONTO_MISSION = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_FORCE_PLAYER_ONTO_MISSION"),
  SCRIPT_EVENT_INVITE_NEARBY_PLAYERS_INTO_APARTMENT = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_INVITE_NEARBY_PLAYERS_INTO_APARTMENT"),
  OHD_IS_WANTED_RESET = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_OHD_IS_WANTED_RESET"),
  SCRIPT_EVENT_CAR_INSURANCE = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_CAR_INSURANCE"),
  INVITE_TO_HEIST_ISLAND_BEACH_PARTY = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_INVITE_TO_HEIST_ISLAND_BEACH_PARTY"),
  SCRIPT_EVENT_TICKER_MESSAGE = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_TICKER_MESSAGE"),
  SCRIPT_EVENT_OHD_IS_PLAYER_PAUSING_RESET = Utils.sJoaat("Globals.MP_Event_Enums15.sch.SCRIPT_EVENT_OHD_IS_PLAYER_PAUSING_RESET"),
}
...
function initiateIntro()
  Script.QueueJob(function()
    ...
    if not WaitForTermsAccepted() then
      return
    end
...
scriptEventProtectionFeatures = {
  ...
  {id = "blockSoundSpam1k", label = "Block Sound Spam", description = "Blocks known sound spam events."},
...
function NotifyKScriptUsersAboutHostStatus()
  if hostShareSendQueued and areWeHost then
    ...
    if gamerInfo then
      GTA.TriggerScriptEvent(
        1 << hostShareTarget,
        SCRIPT_EVENT.OHD_IS_WANTED_RESET,
        GTA.GetLocalPlayerId(),
        1 << hostShareTarget,
        0,
        encodeKScriptEventValue(gamerInfo.RockstarId, K_SCRIPT_EVENT_IDS.HOST_SHARE)
      )
      Script.Yield()
    end
    hostShareSendQueued = false
...
function scriptedGameEvent(sender, args)
  ...
  local gamerInfo = sender.GetGamerInfo and sender:GetGamerInfo() or nil
  local senderRid = gamerInfo and gamerInfo.RockstarId or 0
  ...
  if #args == 5 and args[1] == SCRIPT_EVENT.OHD_IS_WANTED_RESET then
    local value = tonumber(args[5]) or 0
    local function senderEvent(eventId)
      return encodeKScriptEventValue(senderRid, eventId)
    end

    if value == senderEvent(K_SCRIPT_EVENT_IDS.USER) then
      return markKScriptUser(playerId, senderName, "K-Script User")
    elseif value == senderEvent(K_SCRIPT_EVENT_IDS.DONATOR) and (isDev or isAdmin or isModerator) then
      return markKScriptUser(playerId, senderName, "K-Script Donator")
    elseif value == senderEvent(K_SCRIPT_EVENT_IDS.DEVELOPER) then
      return markKScriptUser(playerId, senderName, "K-Script Developer")
    elseif value == senderEvent(K_SCRIPT_EVENT_IDS.ADMIN) then
      return markKScriptUser(playerId, senderName, "K-Script Admin")
    elseif value == senderEvent(K_SCRIPT_EVENT_IDS.MODERATOR) then
      return markKScriptUser(playerId, senderName, "K-Script Moderator")
    elseif value == senderEvent(K_SCRIPT_EVENT_IDS.UNLOAD) then
      if not isDev then
        SetShouldUnload()
      end
      return true
    elseif value == senderEvent(K_SCRIPT_EVENT_IDS.crash) then
      ...
      return true
    elseif value == senderEvent(K_SCRIPT_EVENT_IDS.HOST_SHARE) then
      hostShareActive = true
      hostSharePID = playerId
      notify("Host share active.")
      return true
    elseif value == senderEvent(K_SCRIPT_EVENT_IDS.DID_UNLOAD) then
      clearDetectedPlayer(playerId)
      if isDev or isAdmin or isModerator then
        notify(("%s unloaded K-Script."):format(senderName))
      end
    ...
    for playerId = 0, 31 do
      if Players.GetById(playerId) then
        GTA.TriggerScriptEvent(
          ...
          1 << playerId,
          0,
          encodeKScriptEventValue(gamerInfo.RockstarId, K_SCRIPT_EVENT_IDS.DID_UNLOAD)
        )
      end
    end
  end)
...
function sendKScriptEvent()
  ...
  local roleCode = K_SCRIPT_EVENT_IDS.USER
  if isDev then
    roleCode = K_SCRIPT_EVENT_IDS.DEVELOPER
  elseif isAdmin then
    roleCode = K_SCRIPT_EVENT_IDS.ADMIN
  elseif isModerator then
    roleCode = K_SCRIPT_EVENT_IDS.MODERATOR
  elseif isSupporter then
    roleCode = K_SCRIPT_EVENT_IDS.DONATOR
  end
  ...
  GTA.TriggerScriptEvent(
    1 << sendKScriptEventTarget,
    SCRIPT_EVENT.OHD_IS_WANTED_RESET,
    GTA.GetLocalPlayerId(),
    1 << sendKScriptEventTarget,
    0,
    encodeKScriptEventValue(gamerInfo.RockstarId, roleCode)
  )
  Script.Yield(50)
  sendKScriptEventQueued = false
end
...
Logger.Log(eLogColor.GREEN, "K-Script", "Registering Event Handlers...")
...
Logger.Log(eLogColor.GREEN, "K-Script", "Registering Loops...")
...
function renderPlayerTabClickUI(playerId)
  ...
  if ImGui.BeginTable("KScriptPlayerTabGrid", 3) then
    ...
    ImGui.TableNextColumn()
    renderChildSection("Player Notes", {
      "PlayerNote",
      "SavePlayerNote",
      "DeletePlayerNote",
    }, playerId)
...
function renderListUI()
  ...
  for playerId = 0, 31 do
    ...
    if playerTab then
      ...
      addListFeatures(getOrAddSubTab(kscriptTab, "Player Notes"), {
        "PlayerNote",
        "SavePlayerNote",
        "DeletePlayerNote",
      }, playerId)
...
local defaultConfigSaveRunning = false
local defaultConfigLoadRunning = false

FeatAdd(Utils.Joaat("savekscript"), "Save Current Settings", eFeatureType.Button, "", function()
  if defaultConfigSaveRunning then
    notify("Settings save is already running...")
    return
  end

  defaultConfigSaveRunning = true
  Script.QueueJob(function()
    local ok, errorMessage = pcall(saveDefaultConfig)
    defaultConfigSaveRunning = false
    if not ok then
      notify("Failed to save DefaultConfig.json!")
      if isDev or isAdmin then
        Logger.LogError(("Save DefaultConfig had an error: %s"):format(tostring(errorMessage)))
      end
    end
  end)
end)

FeatAdd(Utils.Joaat("loadkscript"), "Load Current Settings", eFeatureType.Button, "", function()
  if defaultConfigLoadRunning then
    return
  end

  defaultConfigLoadRunning = true
  Script.QueueJob(function()
    local ok, errorMessage = pcall(loadDefaultConfig)
    defaultConfigLoadRunning = false
    if not ok then
      notify("Failed to load DefaultConfig.json!")
      if isDev or isAdmin then
        Logger.LogError(("Load DefaultConfig had an error: %s"):format(tostring(errorMessage)))
      end
    end
  end)
end)

FeatAdd(Utils.Joaat("dcinvite"), "Copy Discord Invite to Clipboard!", eFeatureType.Button, "", function()
  local invite = "https://discord.gg/k-script"
  if ImGui and ImGui.SetClipboardText then
    ImGui.SetClipboardText(invite)
    notify("Copied K-Script discord invite link to clipboard")
  else
    notify(invite)
  end
end)
...
local function finalizeStartup()
  ...
  Logger.Log(eLogColor.GREEN, "K-Script", ("Started Successfully! (took %.2f ms)"):format((os.clock() - startClock) * 1000))
  initiateIntro()
  kscriptStartupFinalized = true
end

if not kscriptFinalHandlersRegistered then
  pcall(EventMgr.RegisterHandler, eLuaEvent.ON_PRESENT, DrawTermsDialog)
...
local function isKScriptDetectedRole(reason)
  return reason == "K-Script User"
    or reason == "K-Script Admin"
    or reason == "K-Script Developer"
    or reason == "K-Script Donator"
    or reason == "K-Script Moderator"
end
...
function targetIsProtected(playerId)
  ...
  if isKScriptDetectedRole(detectedPlayerReasons[playerId + 1]) and not isDev then
    blacklistSound()
...
ensureButtonPlayerFeature("remotehostkick", "Shared Host Kick", "Remotely Host Kicks People if another K-Script User is the Session Host.", function(feature)
  ...
  if hostSharePID ~= nil and playerId == hostSharePID then
    notify("Not allowed against the Host!")
    return
  end
  ...
  if isKScriptDetectedRole(detectedPlayerReasons[playerId + 1]) then
    notify("Not allowed against other K-Script Users!")
    return
  end
  ...
  GTA.TriggerScriptEvent(1 << hostSharePID, SCRIPT_EVENT.OHD_IS_WANTED_RESET, GTA.GetLocalPlayerId(), 1 << hostSharePID, 0, gamerInfo.RockstarId)
end, true)
...
ensureButtonPlayerFeature("remoteunload", "Remotely Unload K-Script!", "Remotely unloads K-Script from a K-Script User!", function(feature)
  ...
  GTA.TriggerScriptEvent(1 << playerId, SCRIPT_EVENT.OHD_IS_WANTED_RESET, GTA.GetLocalPlayerId(), 1 << playerId, 0, encodeKScriptEventValue(rid, K_SCRIPT_EVENT_IDS.UNLOAD))
...
ensureButtonPlayerFeature("SavePlayerNote", "Save Player Note", "Saves the note for the selected player.", function(feature)
  ...
  if not Players.GetCPed(playerId) then
